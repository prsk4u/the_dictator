...
public class DigitalInkMainActivity extends AppCompatActivity implements StrokeManager.DownloadedModelsChangedListener {
    ...
    public void recognizeClick(View v) throws IOException {
        Task<String> ts=strokeManager.recognize(fOut);
    }

    private boolean checkPermission() {
        int result = ContextCompat.checkSelfPermission(DigitalInkMainActivity.this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE);
    ...
    private void requestPermission() {
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(DigitalInkMainActivity.this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            Toast.makeText(DigitalInkMainActivity.this, "Write External Storage permission allows us to create files. Please allow this permission in App Settings.", Toast.LENGTH_LONG).show();
        } else {
            ActivityCompat.requestPermissions(DigitalInkMainActivity.this, new String[]{android.Manifest.permission.WRITE_EXTERNAL_STORAGE}, PERMISSION_REQUEST_CODE);
        }
    }

    public void clearClick(View v) throws IOException {
        strokeManager.reset();
        DrawingView drawingView = findViewById(R.id.drawing_view);
        fOut.write("\n".getBytes());
        drawingView.clear();
